...
public class GlobalTeleOpTestWithIntake extends OpMode {
    ...
    long dpadDownStartTime = 0;
    long dpadUpStartTime = 0;
    ...
    public void workIntake() {
        //dpad Down 
        if(gamepad1.dpad_down) {
            if(dpadDownStartTime == 0) {
              dpadDownStartTime = System.currentTimeMillis();
            }
            long heldTime = System.currentTimeMillis();
            while(heldTime > 200) {
                intake.setPower(-1.0);
            }
        } else {
        ...
        //dpad Up
        if(gamepad1.dpad_up) {
            if(dpadUpStartTime == 0) {
              dpadUpStartTime = System.currentTimeMillis();
            }
            long heldTime = System.currentTimeMillis();
            while(heldTime > 200) {
                intake.setPower(1.0);
